...
#define obboxedge_calc           GS_TOKEN_PASTE(GS_PREFIXED_NAME(obboxedge_calc_),D)
                                          ...
                                          const int rD )
{
  ...
  unsigned d;
  unsigned ntot=n[0]; for(d=1;d<rD;++d) ntot*=n[d];
  fd->ntot = ntot;
  for(d=0;d<D;++d) {
    fd->elx[d] = elx[d];
  }
  ...
  if (rD == 1)
  {
    obboxedge_calc(fd->obb,elx,n,nel,m,bbox_tol);
  }
  else if (D == 3)
  {
